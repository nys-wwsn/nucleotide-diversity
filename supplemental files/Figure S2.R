...
time_series_1 <- 
  ...
  labs(x = "",
       title = expression(paste("Genome region ",Pi[ww])))+
  scale_y_continuous(
...
cor_plot <- dat_state %>%
  ...
  labs(title = expression(paste("Spearman correlation:")),
       subtitle = expression(paste(Pi[ww], "~ case incidence")),
       x = expression(Pi[ww]),
       y = "")
...
# save
png("Supplemental files/_Figure S2.png",
    units = "in",
    width = 8.5, height = 11,
    res = 600)
